const updateSettings = () => {
  // 「詳細設定」画面に遷移
  document
    .querySelector<HTMLButtonElement>('button[ng-click="$ctrl.switchView()"]')
    ?.click()
  ...
  document
    .querySelector<HTMLButtonElement>(
      'input[ng-model="$ctrl.tableRules.useVPCounter"]'
    )
    ?.click()
  // 知っているカードを尊重
  ...
  document
    .querySelector<HTMLButtonElement>(
      'button[ng-click="$ctrl.showKingdomSelection()"]'
    )
    ?.click()

  // 拡張パックをすべて有効に
  for (const element of document.querySelectorAll<HTMLButtonElement>(
    'input.expansion-checkbox'
  )) {
    element.click()
  }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    document
      .querySelector<HTMLButtonElement>(
        'three-valued-button[ng-model="kc.kingdom.colonies"] button'
      )
      ?.click()
    ...
    document
      .querySelector<HTMLButtonElement>(
        'three-valued-button[ng-model="kc.kingdom.shelters"] button'
      )
      ?.click()
  }
  ...
  for (let i = 0; i < 2; i++) {
    document
      .querySelector<HTMLButtonElement>('div[ng-click="$ctrl.addNewSlot()"]')
      ?.click()
  }
  ...
  for (let i = 0; i < 2; i++) {
    document
      .querySelector<HTMLButtonElement>('div[ng-if="$ctrl.canBeNothing()"]')
      ?.click()
  }

  // 「生き方」を含める
  for (const element of document.querySelectorAll<HTMLButtonElement>(
    'div.landscape-type-text.type-W'
  )) {
    element.click()
  }
  ...
  // 終了
  document
    .querySelector<HTMLButtonElement>('input[ng-click="kc.close()"]')
    ?.click()
